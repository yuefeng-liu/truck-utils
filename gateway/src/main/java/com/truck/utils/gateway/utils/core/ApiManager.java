...
import com.truck.utils.gateway.utils.annotation.*;
import com.truck.utils.gateway.utils.define.*;
import com.truck.utils.gateway.utils.entity.*;
import com.truck.utils.gateway.utils.responseEntity.*;
import com.truck.utils.gateway.utils.util.HttpApiProvider;
import com.truck.utils.gateway.utils.util.POJOSerializerProvider;
import com.truck.utils.gateway.utils.util.TypeCheckUtil;
import com.truck.utils.gateway.utils.util.TypeCheckUtil.DescriptionAnnotationChecker;
import com.truck.utils.gateway.utils.util.TypeCheckUtil.PublicFieldChecker;
import com.truck.utils.gateway.utils.util.TypeCheckUtil.SerializableImplChecker;
...
public final class ApiManager {
     ...
     * 填充nameToApi 和 apiInfos Map
     * @param jarfilename jar包名
     * @param apis apiMethodInfo List
     */
            ...
            for (ApiMethodInfo api : apis) {
                ...
                */
                if (api.state == ApiOpenState.OPEN || api.state == ApiOpenState.DEPRECATED) {
                    //将ApiOpenState OPEN DEPRECATED 填充到MAP中
                    apiInfos.put(api.methodName, api);
                    // HttpApiExecuter 为无状态逻辑类的指定函数产生一个代理，代理接口接受字符串数组，转换后调用原函数
     ...
     * 3）验证各种参数 注释的合法性
     * @param clazz             反射出来的接口
     * @param serviceInstance dubbo服务消费者引用服务配置
     * @return
     */
    public static List<ApiMethodInfo> parseApi(Class<?> clazz, Object serviceInstance) {
        //获取注释 ApiGroup (eg:@ApiGroup(name = "user", minCode = 1000000, maxCode = 2000000, codeDefine = UserServiceHttpCode.class, owner = "ren dong"))
        ApiGroup groupAnnotation = clazz.getAnnotation(ApiGroup.class);
        if (groupAnnotation == null) {
            return null;
        }
        try {
            int minCode = groupAnnotation.minCode(); //错误码下限
            int maxCode = groupAnnotation.maxCode(); //错误码上限
            String groupName = groupAnnotation.name(); //ApiGroup名称
            Class<?> returnCodeClass = groupAnnotation.codeDefine(); //错误码定义

            //填充返回码-错误码 RetureCodeContainer
            try {
            ...
            for (Method mInfo : clazz.getMethods()) {
                ...
                if (api != null) {
                        ...
                        }
                        {
                            ...
                            if (a[0].annotationType() != ApiCookieAutowired.class || pInfo.type != Map.class) {
                                TypeCheckUtil.recursiveCheckInputType(clazz.getName(), pInfo.type, pInfo.actuallyGenericType,
                                        new TypeCheckUtil.DescriptionAnnotationChecker(), new TypeCheckUtil.SerializableImplChecker(),
                                        new TypeCheckUtil.PublicFieldChecker());
                            }
                        }
                    ...
                    TypeCheckUtil.recursiveCheckReturnType(clazz.getName(), apiInfo.returnType, apiInfo.actuallyGenericReturnType,
                            new TypeCheckUtil.SerializableImplChecker(), new TypeCheckUtil.DescriptionAnnotationChecker(),
                            new TypeCheckUtil.PublicFieldChecker());

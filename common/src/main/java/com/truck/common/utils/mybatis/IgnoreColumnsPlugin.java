...
public class IgnoreColumnsPlugin extends PluginAdapter {
            ...
            switch (innerClass.getType().getFullyQualifiedName()){
                case "GeneratedCriteria":
                    ...
                    introspectedTable.getAllColumns().forEach(column -> {
                        if(column.getActualColumnName().contains("terminal")){
                            Method terminalContainsMethod = new Method("andTerminalContains");

                            terminalContainsMethod.setReturnType(topLevelClass.getInnerClasses().stream().filter(clazz -> clazz.getType().getShortName().equals("Criteria")).findFirst().get().getType());
                            terminalContainsMethod.setVisibility(JavaVisibility.PUBLIC);

                            terminalContainsMethod.addParameter(new Parameter(new FullyQualifiedJavaType("java.lang.Integer"),"terminal"));
                            terminalContainsMethod.addBodyLine("addCriterion(\"terminal & \" + terminal + \" > 0\");");
                            terminalContainsMethod.addBodyLine("return (Criteria) this;");
                            innerClass.getMethods().add(terminalContainsMethod);
                        }
                    });
    ...
    public boolean clientUpdateByPrimaryKeyWithoutBLOBsMethodGenerated(Method method, Interface interfaze, IntrospectedTable introspectedTable) {
        FullyQualifiedJavaType type = new FullyQualifiedJavaType("com.truck.base.repositories.common.Mapper");
